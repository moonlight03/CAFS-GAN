...
def load_image(img_path,target_shape=128):
    '''
...
def print_SSIM_PNSR(class_path, args):
    ...
    PSNR_all = []
    SSIM_all = []
    img_list = sorted([os.path.join(folder_GT, i) for i in os.listdir(folder_GT)])[:args.test_contentimagenums]
    img_list1 = sorted([os.path.join(folder_Gen, i) for i in os.listdir(folder_Gen) if not i.__contains__('mean')])[:args.test_contentimagenums]
...
def print_L1(class_path, args, epoch):
    ...
    pathLA = sorted([os.path.join(pathA, i) for i in os.listdir(pathA) if not i.__contains__('mean')])[:args.test_contentimagenums]
    pathLA_mean = sorted([os.path.join(pathA, i) for i in os.listdir(pathA) if i.__contains__('mean')])[:args.test_contentimagenums]
    pathLB = sorted([os.path.join(pathGT, i) for i in os.listdir(pathGT)])[:args.test_contentimagenums]
    sum_one_L1 = 0.0
    sum_one_L2 = 0.0
    sum_mean_L1 = 0.0
    sum_mean_L2 = 0.0
...
def print_EdgeLoss(class_path, args):
    ...
    edge_loss = 0.0
    image_re_paths = [os.path.join(RE_Path, i) for i in os.listdir(RE_Path) if not i.__contains__('mean')][:args.test_contentimagenums]
    image_gt_paths = [os.path.join(GT_Path, i) for i in os.listdir(GT_Path)][:args.test_contentimagenums]
...
def print_StyleAndContent(class_path, args):
    ...
    GT_list = sorted([os.path.join(GT_Path, i) for i in os.listdir(GT_Path)])[:args.test_contentimagenums]
    RE_list = sorted([os.path.join(RE_Path, i) for i in os.listdir(RE_Path) if not i.__contains__('mean')])[:args.test_contentimagenums]
    Content_list = sorted([os.path.join(Content_Path, i) for i in os.listdir(Content_Path)])[:args.test_contentimagenums]
...
def print_FID(class_path, args):
    ...
    pathGT = os.path.join(args.data_dir, args.test_dataset, args.target_path)
    l1_loss_file = os.path.join(args.res_dir, "loss.txt")
    a_dataset = FIDDataset(pathA, 'mean', args.test_contentimagenums)
    o_dataset = FIDDataset(pathA, 'one', args.test_contentimagenums)
    d_dataset = FIDDataset(pathGT, None, args.test_contentimagenums)
...
def validateUN(data_loader, networks, epoch, args, additional=None):
    ...
    with torch.no_grad():
        ...
        num_tmp_val = args.train_imagenums
        ...
        for cls_idx in range(len(args.att_to_use)):
            ...
            #####################################
            #Cheating = [573, 615, 122, 581, 573, 615, 122, 581]
            ...
            for j in range(len(x_each_cls)):
                if i % len_t == 0 and j < len_t:
                    ...
                    for content_index, (_, source) in enumerate(val_loader):
                        ...
                        #########################################
                        vec_glyph = C_glyph.moco_attention(glyph_input, glyph_input)
                        vec_effect = C_effect.moco_attention(effect_input,effect_input)
                        #########################################
                        ...
                        if content_index == 0:
                            ...
                            refs_output_one = torch.cat((refs_output, x_res_ema_tmp), 0)
                            refs_output_mean = torch.cat((refs_output, x_res_ema_tmp_mean), 0)
                            vutils.save_image(refs_output_one, os.path.join(epoch_path, '{}_{}_{}_{}_{}.jpg'.format(args.gpu, epoch, content_index, i, j)), normalize=True, nrow=args.val_batch)
                            vutils.save_image(refs_output_mean, os.path.join(epoch_path, '{}_{}_{}_{}_{}_mean.jpg'.format(args.gpu, epoch, content_index, i, j)),normalize=True, nrow=args.val_batch)
                        for k in range(x_res_ema_tmp.size(0)):
                            if content_index * args.batch_size + k + 1 > args.test_contentimagenums:
                                break
                        ...
                        if content_index * args.batch_size + k + 1 > args.test_contentimagenums:
                            break
                    if epoch % 10 == 0 and class_path.__contains__('2_2'):
                        print('print_L1......')
