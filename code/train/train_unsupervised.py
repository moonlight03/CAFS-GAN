...
def trainGAN_UNSUP(data_loader, networks, opts, epoch, args, additional):
    ...
    for i in t_train:  # 1个epoch并不是遍历一边数据集，而是采样1000次，每次batchsize=32
        try:
        ...
        x_ref_idx = torch.randperm(x_org.size(0))  # 0-31的随机数
        ...
        if epoch >= args.separated:
            ...
            s_fake = C.moco(x_fake)  # s'
            s_ref_ema = C_EMA.moco(x_ref)  # s~
