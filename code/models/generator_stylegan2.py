...
try:
    from models.blocks_stylegan2 import LinearBlock, Conv2dBlock, ResBlocks
except:
    from blocks_stylegan2 import LinearBlock, Conv2dBlock, ResBlocks
...
class Generator(nn.Module):
    def __init__(self, img_size=128, sty_dim=64, n_res=2, use_sn=False):
        ...
        self.mlp_t = MLP(sty_dim, 4096, self.nf_mlp, 3, 'none', 'relu')
        self.mlp_g = MLP(sty_dim, 896, self.nf_mlp, 3, 'none', 'relu')
        self.mlp = MLP(sty_dim*2, 4096+896, self.nf_mlp, 3, 'none', 'relu')
    ...
    def decode(self, vec_glyph, vec_effect, vec_source):



        ########################################################################################################################
        # adapt_params = self.mlp(torch.cat([vec_glyph, vec_effect], 1))
        # self.adaptive_param_assign(adapt_params, self.decoder)
        # out = self.decoder(vec_source)

        ########################################################################################################################

        adapt_params = self.mlp(torch.cat([vec_glyph, vec_effect], 1))
        adapt_params_g = self.mlp_g(vec_glyph)
        adapt_params_t = self.mlp_t(vec_effect)
        adapt_params = (adapt_params + torch.cat([adapt_params_t, adapt_params_g], 1))
        self.adaptive_param_assign(adapt_params, self.decoder)
        out = self.decoder(vec_source)
...
class MLP(nn.Module):
    def __init__(self, nf_in, nf_out, nf_mlp, num_blocks, norm, act, use_sn=False):
        ...
        self.model = nn.ModuleList()
        nf = nf_mlp
        self.model.append(LinearBlock(nf_in, nf*2, norm=norm, act=act, use_sn=use_sn))
        for _ in range(num_blocks - 2):
            self.model.append(LinearBlock(nf*2, nf*2, norm=norm, act=act, use_sn=use_sn))
        self.model.append(LinearBlock(nf*2, nf_out, norm='none', act='none', use_sn=use_sn))
        self.model = nn.Sequential(*self.model)
